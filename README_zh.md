...
- **仅安全提炼了 PR #54 的有效核心** - 没有合入高风险原型，而是在保留现有任务状态流转的前提下，新增了独立的 nullable `work_phase` 字段。
- **创建任务时可按标题前缀自动推断工作阶段** - `POST /api/tasks` 现在可识别 `[API]`、`[UI]`、`[コンポーネント]`、`[문서]`、`[组件]`、`[文档]`、`[调试]` 等标签。
- **Task Board 现可直接查看、筛选和编辑工作阶段** - 新增 `Work Phase` 过滤器、任务卡片徽标和内联编辑。
- **PATCH 语义已收紧以避免静默覆盖** - 仅修改标题时不再重新推断或清空 `work_phase`，只有显式传入时才会变更。
- **新功能的多语言支持已补齐** - Task Board UI 支持英文、韩文、日文、中文，服务端前缀自动分类也补充了中文 alias。
- **本次发布明确只包含 PR #54 的安全子集** - admin/local-server 路由、system-console、patch auto-apply 和 status enum 扩展均被刻意排除。
