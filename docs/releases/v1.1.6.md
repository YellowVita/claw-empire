...
- **Orchestration: GitHub-Aware Finalization**
  ...
  - Project policy can now opt into `mergeStrategy.mode: task_branch_pr`, which pushes the task branch and opens a task PR to `dev` instead of folding immediately into the shared `dev` branch.
