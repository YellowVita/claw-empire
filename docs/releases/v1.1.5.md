...
- **Project-Scoped Worktree Enforcement (Orchestration Path)**
  ...
  - If the target `project_path` is not a Git repo, runtime only auto-bootstraps when the project explicitly opts in via `gitBootstrap.allowAutoGitBootstrap: true`.
